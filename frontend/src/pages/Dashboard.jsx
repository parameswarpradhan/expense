...
import NotificationBell from "../components/NotificationBell"; // ✅ Bell import
...
const MetricCard = ({ title, value, colorClass }) => {
  ...
  return (
    <div className="bg-white/5 p-6 rounded-xl shadow-2xl border border-white/10 flex flex-col justify-between hover:bg-white/10 transition duration-300">
      <h3 className="text-sm font-light uppercase text-gray-400 mb-4">
        {title}
      </h3>
              ...
              >
                <div className="flex-1">
                  <p className="text-lg font-medium text-gray-50">
                    {tx.details || tx.description || "No Description"}
                  </p>
                  <p className="text-sm text-gray-400">
                    {new Date(tx.timestamp || tx.date || tx._id).toLocaleDateString(
                      "en-IN",
                      {
                        day: "numeric",
                        month: "short",
